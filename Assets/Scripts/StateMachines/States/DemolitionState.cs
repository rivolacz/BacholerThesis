...
namespace Project.StateMachines.States
{
    public class DemolitionState : BaseState
    {
        ...
        public override void StateUpdate()
        {
            ...
            colliders = colliders.OrderBy(collider => Vector3.Distance(collider.transform.position, unitTransform.position)).ToArray();
            Transform shootingTarget = null;
            foreach (Collider2D collider in colliders)
            {
                ...
                ConstructionSite site = target.GetComponentInParent<ConstructionSite>();
                if (site == null || !site.isBlueprint)
                {
